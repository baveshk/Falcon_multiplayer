# File: server.py
# Description: Server side for managing the 'Falcon_Jets' game
import json
import socket
import threading
import time
...
class Server:
    ...
    def start(self) -> None:
        ...
        # x=self.__server_socket.recv(1024).decode()
        # print(x)
        while True:
            ...
            for current_socket in read_list:
                if current_socket is self.__server_socket:  # If a new client is trying to connet
                    ...
                    if len(self.players) >= 2:  # Checking if the max amount of clients is reached
                        ...
                        self.send_message(client_socket, message)  # Send message only to the new client
                        client_socket.close()
                    else:
